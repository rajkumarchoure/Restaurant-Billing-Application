...
$pdf->Cell(40,6,'Expanse Group',1,0,'L',0);
$pdf->Cell(60,6,'Expense Name',1,0,'L',0);
$pdf->Cell(40,6,'Expense Date',1,0,'L',0);
$pdf->Cell(40,6,'Expense Amount',1,1,'L',0);

$pdf->SetX(5);
$pdf->SetAligns(array('L','L','L','L','R'));
$pdf->SetWidths(array(10,40,60,40,40));
$pdf->SetFont('Arial','',6);
...
$sql_get = mysql_query("select * from expanse where 1=1 order by expanse_id desc");
	...
	{ 
	    ...
	    $total = $row_get['exp_amount'];
		$exp_amount+=$total;
	$pdf->SetX(8);	
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->Row(array($slno++,$group_name,$row_get['exp_name'],$cmn->dateformatindia($row_get['exp_date']),number_format(round($total),2)));
	
	        }
$pdf->SetX(8);
$pdf->SetFont('Arial','B',10);
$pdf->Cell(150,5,'Grand Total (Rs.): ',1,0,'R',0);
$pdf->Cell(40,5,number_format(round($exp_amount),2),1,1,'R',0);
$pdf->Output();
?> 
